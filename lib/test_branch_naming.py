...
from .state import get_worker_worktree_slug
...
class TestBranchNamingConvention(unittest.TestCase):
    ...
    def test_branch_in_worktree_created_with_correct_name(self) -> None:
        ...
        # openniuma 引擎目录的父目录是 POI 项目仓库
        _niuma_dir = Path(__file__).resolve().parent.parent
        _repo_dir = _niuma_dir.parent  # POI 项目根目录

        # 读取 POI 仓库的分支列表（而非 openniuma 引擎的嵌套 git）
        try:
        ...
        self.assertGreater(
            len(feature_branches), 0,
            "测试仓库应有至少一条功能分支（从历史任务创建）"
        )
