...
## 复杂度确认（先做这步！）
快速浏览需求 + ≤5 个相关文件，按以下标准判定：
- **低**（≤5 文件改动，无 DB migration，无新 API）→ 更新 current_phase="FAST_TRACK"，结束会话
- **高**（DB migration + 新 API + 新页面，或 >20 行需求）→ 更新 current_phase="DESIGN"，结束会话
- **中** → 继续
...
3. 在当前 worktree 创建功能分支（worktree 已是 dev_branch HEAD，直接建分支即可）：
   ```bash
...
5. TDD 实现：按 spec 中的步骤逐个完成
6. 每完成一个步骤：门禁检查 → commit → 更新 implement_progress
7. 全部完成后 push，更新 current_phase = "VERIFY"
